...
function histogramData = findEchoes(echoDat,normDat)
...
%of 2.5, so using 2 should be safe.
probeHeight= 1;
...
%     [~,peaks(k)]= max(wave.y);
    %to avoid a bug I encountered in one trace, where the probe peak is
    %forked, and the two forks happen to have exactly the same height.    
    [~,blah]= findpeaks(offsetNorm(:,k),...
        'MinPeakProminence',probeHeight);
    peaks(k)= round(mean(blah));
...
colorsPlain = cellstr([colors colors]'); 

dots = '.......';
    colorsDots = cellstr([colors' dots']);
    colorsDots = vertcat(colorsDots, colorsDots);

dashes = '-------';
    colorsDashes = cellstr([colors' dashes']);
    colorsDashes = vertcat(colorsDashes, colorsDashes);

xes = 'xxxxxxx';
    colorsXes = cellstr([colors' xes']);
    colorsXes = vertcat(colorsXes, colorsXes);

oes = 'ooooooo';
    colorsOes = cellstr([colors' oes']);
    colorsOes = vertcat(colorsOes, colorsOes);
%%%
...
%     plot(locs,5*pks+0.5*(j-1),colorsDashes{mod(j,14)+1});
    
%     figure(2)
%     plot(time,alignedNorm(:,j)+5*(j-1),'Color',[0.8500, 0.3250, 0.0980]);
