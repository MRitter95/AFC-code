...
function [] = colormappingAFC_goodSpacing(config,...
    freqFile, userpath, saveFigs, saveTextTraces, plotprobes)
...
if(nargin<1)
    ...
    freqFile       = input('Path to frequency file','s');
...
for i=1:numfiles
    ...
    if(saveTextTraces)
        name= freqs(i)
        txtFile= ['H:\2020\2020-03\2020-03-13\combMeasurement\textFiles\',...
            num2str(name,'%0.2f'), 'MHz.txt'];
...
%     hold on
%     plot(ffreq, famp+freqs(i)*step, locs, peaks+freqs(i)*step, 'x');
%     xlabel('Time (\mus)');
%     ylabel('Modulation frequency (MHz)');
