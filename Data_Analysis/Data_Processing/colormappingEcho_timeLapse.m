...
function [echovals] = colormappingEcho_timeLapse(config, userpath, fileEnding)
...
echofiles  = dir(['echo*' num2str(fileEnding,'%02d') '.bin']);
[~,idx]    = sort_nat({echofiles.name});
echofiles  = echofiles(idx);
probefiles = dir(['probe*' num2str(fileEnding,'%02d') '.bin']);
...
for i=1:numfiles
    ...
    amp= amp-mean(amp);%include this to remove dc noise
    ...
    timep    = timep - locsp(1);
    locsp    = locsp - locsp(1);
    cutoff   = 0.150; %Use cutoff to reduce noise in data, storage pulse leaks out until 0.07us
...
% print('-f3',['Colormap' num2str(fileEnding,'%02d')], '-dpdf','-r500','-bestfit') % saves storage time info
